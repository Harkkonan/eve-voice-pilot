...
The workbench is separate from the corp-facing Flight Attendant site. It can start the local server, manage an optional local SSH tunnel for operator access to a VM loopback service, run fixed VM service checks, show Git status, check SSO/env readiness without printing secrets, and keep an ignored local action log. That SSH tunnel is not the public hosting path; production hosting should run from the VM. Configure VM access in ignored local JSON at `profiles/flight_workbench.local.json`. Details are in `docs/flight_attendant_workbench.md`.
