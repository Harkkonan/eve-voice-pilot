...
The `Ore Reprocessing` tab estimates mineral output from an ore amount using ESI location, skills, standings, and implants plus local SDE ore/station data. It can rank NPC reprocessing stations where your ESI standing is over 1.5, let you select one for the calculation, and compare the Jita buy-order value of the processed materials with the Jita buy-order value of the unprocessed ore stack. Upwell structure rigs, taxes, and bonuses still need manual overrides because ESI does not expose those settings.
