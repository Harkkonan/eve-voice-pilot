...
- 2026-06-13: Re-opened the official EVE Developer License Agreement, developer license docs, and EVE third-party policy page before release-hardening the Corp Market/Flight Attendant hosted deployment. The changes add proxy/service/backup/smoke templates, HSTS/security-header coverage, bounded ESI retry/cache behavior, admin-only decision-history export/prune/clear controls, and stricter Discord handoff redaction. They add no monetization, no new ESI write scope, no automatic in-game action, no client memory/cache/packet/screen-state reading, and keep the retired Corp Intel Board inactive.
...
- 2026-06-16: Re-opened the official EVE Developer License Agreement, developer license docs, and EVE third-party policy page before adding Intel Pet mission-library browsing and read-aloud briefings. The first slice uses a small hand-authored starter mission file, keeps personal expanded mission libraries ignored under `profiles/`, caches only explicit selected/read mission speech under ignored `cache\speech\`, adds no ESI scope, does not read the EVE client, and does not control gameplay.
- 2026-06-17: Re-opened the official EVE Developer License Agreement, developer license docs, EVE third-party policy page, and Discord webhook docs before adding opt-in Intel Pet Discord channel alert delivery. The slice is disabled by default, dry-run by default, uses a local environment-provided webhook, disables Discord mentions, sends summary-only alert payloads unless matched text is explicitly enabled, rate-limits and dedupes live sends, adds no ESI scope, stores no webhook in committed files, does not restore the retired shared corp intel board, and does not control the EVE client.
