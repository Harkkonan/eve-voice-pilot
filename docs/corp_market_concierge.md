...
- It includes a `PLEX Deal Finder` on the Market Posts tab for public hub checks from 100 through 1,000 PLEX. The sell side emphasizes immediate gross value from public buy-order depth, the buy side shows public sell-order depth, and when ESI returns no PLEX depth the tab falls back to clearly labeled Fuzzwork station aggregate estimates. The tool does not buy, sell, contract, move PLEX, read wallet data, or need an ESI scope.
